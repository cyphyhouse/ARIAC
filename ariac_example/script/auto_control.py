...
def move_agvs(agvObject, dest):
    ...
    if dest == 'near':
        agvObject.cur_pos = a if new_euclidean_dist(a, agvObject.cur_pos) < new_euclidean_dist(b,
                                                                                               agvObject.cur_pos) else b
    else:
        agvObject.cur_pos = a if new_euclidean_dist(a, agvObject.cur_pos) > new_euclidean_dist(b,
                                                                                               agvObject.cur_pos) else b
...
class MoveitRunner():
    ...
    def goto_pose(self, x, y, z):
        ...
        if conveyor_side:
            alpha, beta = find_alphabeta([x - 0.1158, z + 0.1], self.robotObject.pose[
                                                                0::2])  # adjust these values to account for wrist lengths
        else:
    ...
    def gantry_goto_pose(self, pose):
        ...
        # torso rails
        cur_gantry_pose[0] = x + gantryRobot.torso_to_arm * math.sin(
            torso_rotation) + 2  # +2 for difference between joint_value and x-coord
...
class AGV_module():
    ...
    def get_new_loc(self, A):
        ...
        return (0.16 + A.cur_pos[0] - AGV_ROW_SPACE + AGV_ROW_SPACE * math.floor(A.num_items / 3),
                A.cur_pos[1] + AGV_COL_SPACE * (A.num_items % 3))
...
class Follow_points():
    def __init__(self, moveit_runner_kitting, gm):
        ...
        # with open("/home/yubink2/araic_ws/src/ARIAC/nist_gear/config/user_config/sample_user_config.yaml", "r") as f:
        #     data = yaml.load(f)
    ...
    def main_body(self, q, t, agvs):
        ...
        if self.kitting_state == 1:
            ...
            self.moveit_runner_kitting.goto_pose(world_pose.pose.position.x, world_pose.pose.position.y,
                                                 world_pose.pose.position.z + item_height)
        ...
        if self.kitting_state == 3:
            ...
            for i in AGVObjects:
                cur_pose = (
                kitting_arm.get_current_pose().pose.position.x, kitting_arm.get_current_pose().pose.position.y)
            ...
            move_success = self.moveit_runner_kitting.goto_pose(drop_x, drop_y,
                                                                robotObjects[0][0].pose[2])  # mult robot change
            if not move_success:
    ...
    def main_body(self):
        ...
        if self.gantry_state == 2:
            ...
            for i in range(1, 11):
                ...
                print('i str:', str(i))
                target_frame = str(
                    'logical_camera_' + agvObject.name + '_' + station + '_' + self.target + '_' + str(i) + '_frame')
            ...
            item_height = get_item_height(self.target)
            height_buffer = 0.03
            self.moveit_runner_gantry.gantry_goto_pose([world_pose.pose.position.x, world_pose.pose.position.y,
                                                        world_pose.pose.position.z + item_height + height_buffer,
                                                        math.pi / 2])
...
if __name__ == '__main__':
    ...
    if not connectivity(robotObjects):
        print(
            'Missing a link in Conveyor -> Kitting -> AGV -> Gantry pathway. A connected controller cannot be generated.')
        exit()
